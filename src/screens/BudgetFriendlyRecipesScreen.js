...
export default function BudgetFriendlyRecipesScreen() {
  ...
  const loadRecipes = async () => {
    ...
    const newRecipes = await fetchBudgetFriendlyRecipes(offset);
  ...
  return (
    <ScrollView style={{ backgroundColor: "#121212" }}>
      <View style={{ padding: 16 }}>
        ...
        {recipes.map((item, index) => ( // UPDATED
          <TouchableOpacity
            key={index}
        ...
        {hasMore && (
          <TouchableOpacity onPress={loadRecipes}>
            <Text style={{ color: 'yellow', textAlign: 'center', margin: 20 }}>Load More</Text>
          </TouchableOpacity>
        )}
      </View>
    </ScrollView>
